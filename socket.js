...
const socketModule = (io) => {
  io.on("connection", (socket) => {
    ...
    socket.on(
      "message",
      async (roomId, userId, otherId, message, notificationId) => {
        console.log(
        ...
        const isInRoom = Array.from(socketsInRoom).some(
          (socketId) => io.sockets.sockets.get(socketId).userId === otherId
        ); // 해당 방에 otherId가 있는지 확인
